<template>
  <div class="lg:pl-40 lg:pr-20 p-4 ">
    ...
    <!-- table -->
    <q-table flat bordered :visible-columns="visibleColumns" title="Facturas" :rows="rows" :columns="columns"
      row-key="index" virtual-scroll class="inventTable h-[450px] lg:h-[680px] rounded-2xl"
      v-model:pagination="pagination" :rows-per-page-options="[0]">
      <template v-slot:header="props">
        <q-tr :props="props">
          <q-th v-for="col in props.cols" :key="col.name" :props="props">
            <q-icon v-if="col.icon" :name="col.icon" size="22px" color="white"/>
            {{ col.label }}
          </q-th>
        </q-tr>
      </template>
      ...
      <template v-slot:top-right>
        <div>
          <q-select v-model="visibleColumns" multiple dense label="Campos" options-dense
      ...
      <template v-slot:body-cell-options="props">
        <q-td :props="props">
          <div class="flex flex-row justify-center ">
            <!-- edit -->
            <button class="button editBtn bg-white"
              @click="(index = props.row._id), goInfo(props.row), modalCreaEdit(2)">
            ...
            <!-- delet -->
            <button class="button" id="delet" @click="Delete(props.row)">
            ...
            <!-- visualizar -->
            <button class="button" @click="SumCant(props.row)">
  ...
  <q-dialog v-model="dialog" class="p-0">
    <div class="mr-2  xs:w-[90%] min-h-[70vh] rounded-[20px] bg-white p-6">
