<template>
  ...
  <div class="p-4 sm:p-6 md:p-10 lg:p-16 w-full max-w-[1600px] mx-auto pb-24 lg:pb-10">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
      <div>
        <h1 class="text-[#1a2332] text-2xl sm:text-3xl md:text-4xl font-black tracking-tight flex items-center gap-2">
          <div class="w-2 h-6 sm:h-8 bg-green-600 rounded-full shrink-0"></div>
          DASHBOARD
        </h1>
        <p class="text-gray-400 text-xs sm:text-sm mt-0.5 ml-4 italic">Descripción General</p>
      </div>
      
      <div class="flex items-center gap-3 bg-white p-2 pr-4 sm:pr-6 rounded-2xl shadow-sm border border-gray-100 w-full sm:w-auto">
        <div class="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-[#1a2332] flex items-center justify-center text-white font-bold text-sm shrink-0">
          {{ storeLogin.Name?.charAt(0) }}
        </div>
        <div class="flex flex-col min-w-0">
          <span class="text-sm font-bold text-[#1a2332] truncate">{{ storeLogin.Name }}</span>
          <span class="text-[10px] text-gray-400 leading-none truncate">{{ storeLogin.Email }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="mb-10">
      <div class="flex items-center gap-3 mb-6">
        <div class="w-1.5 h-6 sm:h-8 bg-[#1a2332] rounded-full"></div>
        <h2 class="text-lg sm:text-xl font-bold text-[#1a2332] tracking-tight uppercase">Análisis de Flujo</h2>
      </div>

      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">

        <!-- Columna 1: Productos Globales Más Vendidos -->
        <div class="lg:col-span-6 bg-[#1a2332] p-5 sm:p-8 md:p-10 rounded-2xl sm:rounded-[2.5rem] shadow-xl text-white flex flex-col justify-start">
          <h3 class="font-bold mb-6 opacity-80 text-center uppercase text-xs tracking-[2px]">
          ...
          <div class="flex flex-col gap-4 overflow-y-auto max-h-[350px] pr-2 custom-scroll">
            ...
            <div 
              ...
              class="flex flex-col gap-1.5 border-b border-gray-700/50 pb-3 last:border-none"
            >
              <div class="flex justify-between items-center text-xs sm:text-sm gap-2">
                <div class="flex items-center gap-2 min-w-0 flex-1">
                  ...
                  <span class="font-medium truncate" :title="product.Name || product.Nombre">
        ...
        <div class="lg:col-span-6 bg-slate-900 border border-slate-800/80 p-5 sm:p-8 rounded-2xl sm:rounded-[2.5rem] shadow-2xl text-slate-100 flex flex-col justify-start">
          
          <!-- Encabezado con Icono y Selector -->
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-800">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h3 class="font-semibold text-xs sm:text-sm tracking-wide text-slate-200 uppercase">
                  Rendimiento por Categoría
                </h3>
                <p class="text-[11px] sm:text-xs text-slate-400">Top de productos con mayores ventas</p>
              </div>
            </div>

            <div v-if="categoriesData.length > 0" class="relative w-full sm:w-auto">
              <select 
                v-model="selectedCategoryId"
                class="appearance-none bg-slate-800/90 text-xs text-indigo-300 font-medium pl-3.5 pr-8 py-2 rounded-xl border border-slate-700/80 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all cursor-pointer w-full shadow-inner"
              >
          ...
          <div class="flex flex-col gap-3 overflow-y-auto max-h-[360px] pr-1.5 custom-scroll">
            ...
            <div 
              ...
              :key="product.IdProduct || index" 
              class="group bg-slate-800/40 hover:bg-slate-800/80 border border-slate-800 hover:border-slate-700/80 p-3 sm:p-3.5 rounded-2xl transition-all duration-200 flex flex-col gap-2.5"
            >
              <div class="flex justify-between items-center gap-2">
                <div class="flex items-center gap-2.5 min-w-0 flex-1">
                  <span 
                    class="w-6 h-6 rounded-lg text-xs font-bold flex items-center justify-center shrink-0"
                    :class="{
                ...
                <div class="text-right shrink-0">
                  <span class="text-[11px] sm:text-xs font-bold text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-lg border border-emerald-500/20">
                    {{ product.TotalUnits || product.qty || 0 }} <span class="text-[9px] sm:text-[10px] font-normal opacity-80">{{ product.unit_measurement || 'unds' }}</span>
                  </span>
                </div>
              </div>
    ...
    <FooterUs class="mt-8 sm:mt-12 py-6 border-t border-gray-100" />
  </div>
</template>
